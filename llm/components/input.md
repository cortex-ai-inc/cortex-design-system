...
- **cortex-support-front** (primary, branded) — `h-10`, `rounded-sm`, `ghost-border-20` utility, `focus:ring-2`. This is the canonical Cortex form control documented below.
- **cortex-coder-front** — a near-stock shadcn variant: `h-9`, `rounded-md`, `border-input`, `bg-transparent`, `focus-visible:ring-[3px]`, plus `file:` utilities and `aria-invalid` styling. See "Coder-front variant" below.
...
Primary source — `cortex-support-front/.../ui/input.tsx`, base className quoted verbatim:

```tsx
"flex h-10 w-full rounded-sm border ghost-border-20 bg-surface-dim px-3 py-2 text-body-sm text-on-surface placeholder:text-on-surface-variant/50 focus:outline-none focus:ring-2 focus:ring-k-primary-container/50 disabled:cursor-not-allowed disabled:opacity-50"
```

`Textarea` (same source) shares the identical base, swapping `h-10` for `min-h-[80px]`:
...
- **Focus**: `focus:outline-none` removes the native outline; `focus:ring-2 focus:ring-k-primary-container/50` adds the blue focus ring. (Note: `focus:`, not `focus-visible:`.)
...
| Class | Description |
|---|---|
| `min-h-[80px]` | Minimum height 80px (replaces `h-10`) |
...
- Sizing: `h-9`, `rounded-md`, `px-3 py-1`, `text-base md:text-sm` (vs. `h-10`, `rounded-sm`, `px-3 py-2`, `text-body-sm`).
- Surface/border: `bg-transparent` + `border-input` (vs. `bg-surface-dim` + `ghost-border-20`); also `dark:bg-input/30`.
- Focus: `focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]` (vs. `focus:ring-2 focus:ring-k-primary-container/50`).
