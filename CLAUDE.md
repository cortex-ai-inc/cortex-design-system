...
| Tabs | Tabs | underline-style on active |
...
| StatusDot | No (custom) | 6 statuses, 2 sizes, pulse animation |
...
1. **Never use `rounded-full`** — Badges use `rounded-sm` (4px) or pill shapes in cortex-coder-front.
2. **No 1px borders between major sections** — Use tonal shifts (different surface colors).
3. **Never exceed 6px border-radius** — Despite shadcn defaults, cortex-coder-front enforces max 6px.
4. **Icons always `w-4 h-4` with `strokeWidth={1.5}`** — Standard lucide-react sizing.
5. **No modals for create/edit forms** — Always use dedicated pages. Dialog only for AlertDialog confirmations.
6. **Toggle view/edit via `?mode=edit`** query parameter on detail pages.
7. **Form width capped at `max-w-5xl`** — Editor pages at `max-w-6xl`.
8. **Form grid**: `grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5`.
9. **Page headers**: `flex flex-col gap-3 sm:flex-row` (stack on mobile).
10. **Body base**: `bg-surface-dim text-on-surface antialiased font-inter` at 13px.
